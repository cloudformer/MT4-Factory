...
class SimpleBacktester:
    ...
    def run(self, strategy_code: str, data: pd.DataFrame) -> Dict:
        """
        运行回测
...
def generate_historical_data(symbol: str = "EURUSD", bars: int = 1000) -> pd.DataFrame:
    """
    生成模拟历史数据（未来会从MT5获取真实数据）
    ...
    dates = pd.date_range(end=end_date, periods=bars, freq='h')  # 小写h
